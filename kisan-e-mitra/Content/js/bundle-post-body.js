...
async function getWaveBlob(
    blobData, as32BitFloat, arrayBuffer, contextOptions = undefined
) {
    const audioContext = new AudioContext(contextOptions);

    const audioBuffer = await audioContext.decodeAudioData(arrayBuffer);

    //let audioBuffer = await _getAudioBuffer(blobData, contextOptions);
...
(function () {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function startAutoVisualizer() {
            ...
            const bufferLength = 256; // Number of bars
            const dataArray = new Uint8Array(bufferLength);
            const targetArray = new Uint8Array(bufferLength);

            let lastTime = 0;
            const fps = 10; // Frames per second, adjust this value to control speed
            ...
            requestAnimationFrame(drawAutoBars);
        }
